...
class WebSocketTracker extends Tracker {
  ...
  _onAnnounceResponse (data) {
    ...
    //added by liuxi, check response to determine if it's serverPeer and serverPeerNo
    let peerTypeTransfer = false

    if (data.serverPeer != null) {
      if (this.client.serverPeerNo !== data.serverPeer){
        peerTypeTransfer = true
      }
      this.client.serverPeerNo = data.serverPeer
    }
    ...
    //added by liuxi, when detect peer type changes, clear all existed peers
    if (peerTypeTransfer){
      for(peer of this.alivePeers){
    ...
    if (data.answer && data.peer_id) {
      ...
      if (peer) {
        ...
        this.alivePeers.push(peer)  //added by liuxi
